...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let calcAngle = Math.atan2(y2, x2) - Math.atan2(y1, x1);
  calcAngle = calcAngle < 0 ? (calcAngle += 2 * Math.PI) : calcAngle;
  return calcAngle;
}
...
function getSumOfDigits(num) {
  let count = 0;
  let sum = 0;
  while (Math.floor(num / 10 ** count)) {
    sum += Math.floor(num / 10 ** count) % 10;
    count += 1;
  }
  return sum;
}
...
function getSumOfNumbers(...args) {
  return +[...args].reduce((elem, sum) => elem + sum, 0).toFixed(1);
}
...
function getRandomInteger(min, max) {
  ...
  const RandomRaw = min + Math.random() * upperBound;
...
function getCountOfOddNumbers(number) {
  const count = Math.abs(
    number > 0 ? Math.floor(number / 2) : Math.round(number / 2)
  );
  return number % 2 ? count + 1 : count;
}
